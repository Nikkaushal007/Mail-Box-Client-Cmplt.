        ...
        if (receiver.current.value.length > 0 && mailBody.current.value.length > 0 && subject.current.value.length > 0) {
            ...
            try {
                let responce = await fetch(
                    `https://mail-box-client-7061b-default-rtdb.firebaseio.com/mail/${removeSpecialChar(receiver.current.value)}.json`,
                    {
                        method: 'POST',
                ...
                if (responce.ok) {
                    ...
                    try {
                        let responce = await fetch(
                            `https://mail-box-client-7061b-default-rtdb.firebaseio.com/sentmail/${user}.json`,
                            {
                                method: 'POST',
